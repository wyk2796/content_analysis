...
class TrainData(object):
    ...
    def add_content_from_file(self, path):
        with open(path, encoding='utf-8', mode='r') as i_stream:
            data = i_stream.readlines()
            del data[0]
            for line in data:
                content = line.strip().split(sep='\t')
                if content != '' and content[0] not in self.label_map:
                    try:
                        if len(content) == 6:
                            self.label_map[content[0]] = ContentLabel(content[1],
                                                                      content[2],
                                                                      content[3],
                                                                      content[4],
                                                                      content[5])
...
if __name__ == '__main__':
    ...
    # rt.RNN_train_2(train_data, wc, re_train=False)
